...
class TSDFVolumeTorch:
    ...
    """
    @staticmethod
    def get_volume_setting(cfg):
        assert "voxel_size" in cfg and "vol_bounds" in cfg
        voxel_size = cfg["voxel_size"]
        vol_bnds = np.array(cfg["vol_bounds"]).reshape(3, 2)
    ...
    def __init__(self, cfg):
        ...
        margin = cfg["margin"]
        fuse_color = cfg["fuse_color"]
        voxel_dim, origin, voxel_size = self.get_volume_setting(cfg)
    ...
    def integrate(self, depth_im, cam_intr, cam_pose, obs_weight, color_img=None):
        ...
        if len(depth_im.shape) == 3:
            depth_im = depth_im.squeeze(2)   # input depth image is HxWx1
    ...
    def render_model(self, c2w, intri, imh, imw, near=0.5, far=5., n_samples=192):
        ...
        if isinstance(c2w, np.ndarray):
            c2w = torch.from_numpy(c2w).to(self.device).float()
        if isinstance(intri, np.ndarray):
            intri = torch.from_numpy(intri).to(self.device).float()
